...
# attempt to get data files from OSF project and populate the /data directory
# THIS IS NOT USED by default. See populateData function
PopulateData_osf <- function(){
  library(osfr)
  library(dplyr)
  
  if (length(list.files(path = './data2')) < 3) {
    osfProject <- osf_retrieve_node('mx5u2')
